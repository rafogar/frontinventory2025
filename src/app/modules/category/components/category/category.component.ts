...
export class CategoryComponent implements OnInit {
  ...
  openCategoryDialog() {
    ...
    dialogRef.afterClosed().subscribe((result: any) => {
      ...
      if (result == 1) {
        this.openSnackBar('Categoría creada', 'Exitosa');
        this.getCategories();
      }else if(result == 2){
  ...
  edit(id:number, name: string, description: string){
    ...
    dialogRef.afterClosed().subscribe((result: any) => {
      if (result == 1) {
        this.openSnackBar('Categoría actualizada', 'Exitosa');
        this.getCategories();
      }else if(result == 2){
  ...
  delete(id:any){
    ...
    dialogRef.afterClosed().subscribe((result: any) => {
      if (result == 1) {
        this.openSnackBar('Categoría Eliminada', 'Exitosa');
        this.getCategories();
      }else if(result == 2){
  ...
  exportExcel(){
    ...
    this.categoryService.exportCategorie()
    .subscribe( (data:any) => {
      let file = new Blob([data], {type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'});
      let fileUrl = URL.createObjectURL(file);
      var anchor = document.createElement("a");
      anchor.download = "categories.xls";
      anchor.href = fileUrl;
      anchor.click();
